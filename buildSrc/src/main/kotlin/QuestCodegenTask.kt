...
abstract class QuestCodegenTask : DefaultTask() {
    ...
    fun generate() {
        ...
        for (q in quests) {
            ...
            val trackerList = trackers.joinToString(", ") { it.toJava() }
            constants += "    public static final QuestId $name = " +
                    "new QuestId($qid, \"${escape(title)}\", List.of($trackerList));"
        }
    ...
    /** One tracked variable and its id space; mirrors quest-core's TrackerVar. */
    private data class Tracker(val kind: String, val id: Int) {
        fun toJava(): String = "TrackerVar.$kind($id)"
    }

    @Suppress("UNCHECKED_CAST")
    private fun readTrackerOverrides(file: File): Map<Int, Tracker> {
        ...
        for (c in chains) {
            ...
            // The kind is required: a varp and a varbit with the same number are different
            // variables, and guessing one is what made the tracker read the wrong space.
            val kind = when (tracker["kind"]) {
                "varp" -> "varp"
                "varbit" -> "varbit"
                else -> throw GradleException(
                    "action_chains_sample.json: quest $qid tracker ${tracker["id"]} has kind " +
                        "'${tracker["kind"]}'; expected \"varp\" or \"varbit\"")
            }
            out[qid] = Tracker(kind, tid)
        }
        return out
    }
    ...
    private fun resolveTrackerVars(
        ...
        return firstIds(varbits, "varbit") + firstIds(varps, "varp")
    }

    private fun firstIds(rows: List<List<Any?>>, kind: String): List<Tracker> =
        rows.mapNotNull { row -> (row.firstOrNull() as? Number)?.toInt()?.let { Tracker(kind, it) } }
