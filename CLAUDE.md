...
- **Wire protocol version**: `Layout.PROTOCOL_VERSION` (currently `21`) must equal `kProtocolVersion` in `NXTLibrary/src/ipc/SharedLayout.h` (and the `static_assert(nxt::ipc::kProtocolVersion == 21, ...)` in `NXTDebugger/src/attach/Session.h`). `SharedRegion.open()` validates and refuses mismatched versions. `core/.../shm/LayoutWireOffsetsTest` pins the version, the dynamic-region block offsets and `SNAPSHOT_SIZE` as hardcoded literals so a cap edit on either side of the wire makes the two disagree loudly.
...
- **Entity facing** (v21): `Npc` / `Player` / `LocalPlayer` carry an `Orientation` decoded from a `u16` on each row (`Layout.NPC_ORIENTATION_OFFSET` / `PLAYER_ORIENTATION_OFFSET` / `LP_ORIENTATION_OFFSET`). `0xFFFF` is unknown; anything else outside `0..16383` is a producer bug that the decode path (`OrientationWireDecoder`, one per `SharedRegion`) turns into unknown plus one WARN per session. It never throws out of a snapshot read, while `new Orientation(bad)` does throw. The zero offset `Orientation.NORTH_RAW = 8192` comes from static analysis and awaits a live check. Compare facings with `isSameFacingAs` (one-unit tolerance), never `equals`: the client's conversion truncates, so an angle can read back one unit short.
...
- **Key triggers** (`ActionTypes.COMPONENT_TRIGGER` 5003, trigger type 10): `param3` is `KeyStroke.packed()` = `(int16 keyCode << 16) | uint16 keyChar`, unpacked by `DispatchComponentTrigger` in `NXTLibrary/src/game/Actions.cpp`. On the raw wire a bare key code is read as a *character* (`84` types a `T`), so `fireComponentTrigger` keeps its documented key-code form by normalising a key-trigger `arg` with a zero high half to `(arg << 16)` (`KeyStroke.keyTriggerArg`) and passing a packed one through; `fireKeys` sends packed strokes unchanged. Printable characters travel in the char half (`KeyStroke.character(c)` = `(-1, c)`); Enter/Backspace/Escape travel in the code half (`(84|85|13, 0)`). `api.inputDialog()` (`api/input/InputDialog`) targets MESLAYER **1469:4**, not 1469:0, which has no triggers. It reads the mode from varc 5 and the text from varc-string 2506, and it validates against the per-mode limits before typing. Additive: no `PROTOCOL_VERSION` bump.
...
- **Debug drawing** (`api/draw/`, `api.draw()`): the nine `debug_draw_*` / `highlight_component` handlers in `NXTLibrary/src/rpc/Handlers.cpp`, bound by `core/impl/DrawCodec.java` (encode/decode) and `GameAPIImpl`'s `DrawAPI` methods. **Additive over the RPC pipe — no `PROTOCOL_VERSION` bump**; the overlay store is not in shared memory. Caps are mirrored into `api/draw/DrawLimits.java` from `NXTLibrary/src/overlay/DrawTypes.h` (`kMaxDrawCmds`, `kMaxKeyChars`, `kMaxTextSlots`, `kPolySlotPoints`, `kMaxDrawCoord`, `kMaxDrawExtent`, `kMaxResolveTargets`) plus `kDrawBatchMax` in `Handlers.cpp` — change a cap on the producer and change it here in the same logical change. Four wire behaviours the API is deliberately loud about rather than smoothing over, because each one lets a naive wrapper lie to a scripter:
  ...
  - **`z` used to be the one limit the producer did not enforce. It is now enforced on both sides, and this entry is the record of the change.** Through phase 1 `ResolveStyling` narrowed with a plain cast, so `z(100000)` was *silently truncated* to `-31072` and repainted in a different order — no error, no `dropped`, nothing in `debug_draw_list` to show for it. The producer now range-checks it (`z must be -32768..32767`) and gives `color` the same treatment. `DrawLimits.MIN_Z`/`MAX_Z` are still validated in `DrawStyle`'s compact constructor, and that is still worth having — it moves the failure to the call site with a stack trace instead of a round-trip — but it now *agrees with* a producer refusal rather than substituting for a missing one. `LiveHighlightSmokeTest.anOutOfRangeZ_isRefusedByTheProducerRatherThanNarrowed` is what keeps this paragraph true rather than remembered; it was stale here, in `DrawLimits.MIN_Z`'s javadoc, in `DrawStyle`'s error message and in `DrawCommandTest` for one release before anyone checked.
  ...
  - **The three world highlights are bound, and they are the reason `DrawCommand` is split into `Primitive` and `Highlight`.** `highlight_entity` / `highlight_tile` / `highlight_area` back `draw.npc/player/self/tile/area(...)` via `DrawAPI.drawHighlight` + `drawHighlights`, encoded by `DrawCodec.encodeHighlight` / `highlightMethod`. **`highlight_component` is deliberately NOT bound**: `DrawKind.COMPONENT` is spellable on `debug_draw_set`, so `draw.component(...)` already gets the same per-tick re-resolution *and* can ride a batch, which the semantic call cannot. Binding it would add a second, strictly worse spelling.
    - **A highlight cannot be batched, and the type system is what says so.** `debug_draw_set_batch` applies `debug_draw_set` items, whose `kind` cannot name `entity` or `tile` (`ParseDrawKind` omits both on purpose). `drawSetBatch` therefore takes `List<DrawCommand.Primitive>`, so a batched highlight is a compile error rather than a producer-side `unknown kind`. `DrawFrame` still accepts both families: it batches the primitives and sends one `highlight_*` call each, which is why its javadoc no longer claims "one round-trip" unqualified.
    - **A refused highlight is counted, not thrown, inside a frame** — the contract a frame already gave for a refused primitive, and the reason `RpcRemoteException` exists. A highlight is its own call, so the producer reports a refusal as an error envelope rather than a `dropped` count; `GameAPIImpl.sendOneHighlight` catches *that subclass only*, so a dead pipe still propagates instead of becoming a silent `dropped`.
    - **Auto keys are contract and the entity ones carry no padding**: `npc:<index>`, `player:<index>`, bare `self`, `tile:<x>:<y>:<plane>`, `area:<x>:<y>:<w>:<h>:<plane>`. The plane and the extent are in the tile/area keys because a key is an identity and a set *replaces* — two tiles on different floors are two tiles. A stale producer comment above `Handle_HighlightEntity` still says `npc:<index>:0`; the code and `PROTOCOL.md` do not, and `LiveHighlightSmokeTest` is what decides between them.
    - **The tile helpers speak TILES; a `space: "world"` primitive speaks sub-tiles.** `draw.tile(key, 3200, 3200)` sends `3200`, while `draw.rect(...).world()` at the same place sends `3200 * SUBTILE_SCALE`. The producer's handler is the one place the two meet. `DrawLimits.MAX_WORLD_TILE` is derived from `MAX_WORLD_COORDINATE` rather than written down, so the two cannot disagree.
    - **`plane` is honoured by refusal for tile and area, and entity is the exemption.** The producer's only ground-height source is the local player's elevation, so a tile or area on another floor is accepted, stored, and resolves `unavailable` — it draws nothing. An entity carries its own height, reads its *fractional* scene position (so it tracks smoothly rather than jumping a tile), and sends no `plane` at all. That asymmetry is why `draw.npc(...)` is the thing to reach for and why the encoder omits `plane` for an entity rather than sending one the producer ignores.

  **`DrawSpace.WORLD` works as of the phase-3 producer**, and the signatures shipped before it did without needing to change — which is what putting the field on the wire early bought. The live test that pinned the old rejection is what noticed: it was an `assertThrows`, so the feature landing made it fail. It has been repointed at `poly`, which world space still refuses (its points live in a side slot with no room for a projected copy), so the "an unsupported space fails with its *own* message" property keeps a subject; the positive case is asserted separately. **Two coordinate bounds now, not one** — `DrawLimits.MAX_SCREEN_COORDINATE` (2^20) gates what a screen command sends *and* what a world command projects to a tick later, while `MAX_WORLD_COORDINATE` (2^23) gates what a world command sends. The world bound is the whole `int16` tile space at `SUBTILE_SCALE`, so a tile read from a snapshot can never exceed it; one bound for both would have refused everything past tile 4096 for failing a limit about pixels. **That known gap is now closed**: `draw.npc/player(...)` used to mark a *position* rather than an entity and sent no `plane`, so a box stayed put when the NPC walked and an upstairs NPC was marked on plane 0. They now send `highlight_entity` and track. The promise their javadoc made — that binding the semantic highlights would change *what they send* rather than *how they are called* — held: the names and arguments are unchanged, and only the return type moved (to `HighlightBuilder`, which drops `screen()`/`world()`/`closed()` because a highlight has no such choice). `drawClearAll()` is pinned to `scope: "mine"` and the producer's `"all"` is deliberately unreachable from a script.
