...
- **Key triggers** (`ActionTypes.COMPONENT_TRIGGER` 5003, trigger type 10): `param3` is `KeyStroke.packed()` = `(int16 keyCode << 16) | uint16 keyChar`, unpacked by `DispatchComponentTrigger` in `NXTLibrary/src/game/Actions.cpp`. A bare key code in `param3` is read as a *character*: an old-style "Enter" of `84` types a `T`. Printable characters travel in the char half (`KeyStroke.character(c)` = `(-1, c)`); Enter/Backspace/Escape travel in the code half (`(84|85|13, 0)`). `api.inputDialog()` (`api/input/InputDialog`) targets MESLAYER **1469:4**, not 1469:0, which has no triggers. It reads the mode from varc 5 and the text from varc-string 2506, and it validates against the per-mode limits before typing. Additive: no `PROTOCOL_VERSION` bump.
